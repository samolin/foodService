...
def create_checkout_session_order(request, id):
    ...
    checkout_session = stripe.checkout.Session.create(
       ...
       mode='payment',
       success_url=main_domain + "/orders" + '/success/' + "?session_id={CHECKOUT_SESSION_ID}",
       cancel_url=main_domain + "/orders" + '/failed/' + "?session_id={CHECKOUT_SESSION_ID}",
    )
...
def payment_success(request):
    ...
    if Payment.objects.filter(transaction_id=session_data['id']):
        ...
        payment.save()
        # update order
        order = Order.objects.get(user = request.user, order_number = line_items.data[0]['description'])
